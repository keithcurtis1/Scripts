var tokenAction = tokenAction || (function () {
    ...
    var version = '0.3.6',
        ...
        createRepeating = function (name, pattern, id, usename) {
            ...
            _.each(repeatingAttrs, function (attr) {
                var repeatingId = attr.get('name').split('_')[2],
                    ...
                    //5e legendary and mythic action renamer
        ...
        createPF2Spell = function (id) {
            ...
            if (allNormalSpellsText) { allNormalSpellsText = '**Normal Spells**\n' + allNormalSpellsText };
            //log('allNormalSpellsText is ' + allNormalSpellsText); // put this into chat menu when all are done
        ...
        sendChat("TokenAction", "/w " + msg.who + " Created Token Actions for " + a.get('name') + ".");
                ...
                _.each(char, function (a) {
                    ...
                    if (parseInt(isNpc(a.id)) === 1) {//PF2 Sort repeating read from here
                        //log('These are the commands that are being read 3');
                        if (args.includes("init")) {
                            createAbility('Init', "%{" + a.id + "|npc_init}", a.id);
                        }
                        if (args.includes("checks")) {
